...
function refreshPage() {
  // Force a fresh Firestore retrieval, then redraw the current SPA screen.
...
function openStudentBooks(studentId) {
  setSelectedStudent(studentId);
  spaNavigate("student-books", { student: studentId });
}
      ...
      return `
        <div class="checkout-date-group">
          ...
          ${dateBooks.map(book => {
            ...
            const actionHtml =
              (!returnedMode && !adminMode)
                ? `<button
                     class="book-read-action ${book.bookRead ? "mark-unread" : "mark-read"}"
                     type="button"
                     onclick="event.stopPropagation(); toggleBookReadStatus('${book.lendingId}', ${book.bookRead ? "false" : "true"})">
            ...
            return `
              <div class="book-item ${returnedMode ? "returned-book" : ""}">
